...
export const createProject = async (req: Request, res: Response) => {
    ...
    if (images.length < 2 || !productName) {
        return res.status(400).json({ msg: 'Please upload atleast 2 images' })
    }
    ...
    // Deducting credits for image generation
    ...
    try {
        ...
        const model = 'gemini-3-pro-image-preview'; // or use this 'gemini-2.5-flash-image'
        ...
        const prompt = {
            text: `Combine the person and prodcut into a realistic photo. Make the person naturally hold or use the product. Match lighting, shadows, scale and perspective. Make the person stand in professional studio lighting. Output ecommerce-quality photo realistic imagery ${userPrompt}`
        }

        //Generating image using the ai model
        ...
        if (tempProjectId!) {
            //update project status and error msg
...
export const createVideo = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const prompt = `Make the person showcase the product which ${project.productName} ${project.productDescription && `and Product Description: ${project.productDescription}`}`
